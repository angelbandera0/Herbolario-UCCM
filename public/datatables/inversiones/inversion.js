...
<div class="btn-group btn-group-sm" style="float: none;">
  <button id="btn-edit" type="button" class="tabledit-edit-button btn btn-primary waves-effect waves-light active"
    style="float: none;margin: 5px;" data-toggle="modal"
          data-target="#edit-inversion" onclick="editar(this)">
  ...
  <button type="button" class="tabledit-edit-button btn btn-success waves-effect waves-light active"
    style="float: none;margin: 5px;" data-toggle="modal"
          data-target="#details_inversion" onclick="detalles(this)">
...
$(document).ready(function () {
    ...
    $.fn.dataTable.ext.search.push(
        function (settings, data, dataIndex) {
            ...
            var cantidad = parseFloat(data[4]) || 0; // use data for the precio column
            var costo = parseFloat(data[5]) || 0; // use data for the precio column
            //var causa = data[1];
    ...
    mytable = $('#try').DataTable({
        ...
        "columns": [
            ...
            {"data": "productor_asociado.nombre"},
            {"data": "cantidadc"},
            {"data": "costo"},
            {"data": "valor_total"},
        ...
        "createdRow": function (row, data, index) {
            ...
            let auxi=($('td', row).eq(2).text());
            if(!produc.includes(auxi))
                produc.push(auxi);
            let auxii=($('td', row).eq(3).text());
            if(!productores.includes(auxii))
                productores.push(auxii);
            $('td', row).eq(6).addClass('text-c-blue');
